...
from denis_regex_helper import cluster_1, cluster_2, cluster_NOT_titles, cluster_NOT_abstracts
...
def search_df(df, date_max=2020.03, date_min=2005.00, field="TiAbs", name="",ignorecase=True):
    #
    ...
    def regex_filter_titles(val):
        ...
        for reg in cluster_NOT_titles:
            if ignorecase:
                res = re.search(reg, val.strip(),re.IGNORECASE)  # get rid of genetics related search results
            else:
                res = re.search(reg, val.strip())
            if res:
                # print(val)
    ...
    # def regex_filter_c3(val):  # filter for cluster 1. Returns True as soon as the first regex matches. If there is no match then the function returns a False, leading the record to be dropped
    #
    #     found = False
    #
    #     for reg in cluster_3:
    #
    #         result = re.search(reg, val.strip(), re.IGNORECASE)
    #         if result:
    #             found = True
    ...
    #         return False
    #
    # df = df[df[field].apply(regex_filter_c3)]  # apply function defined above
    # df_copy.drop(df.index, inplace=True)
    # print("Dropped {} records via the third search cluster.\nCurrent nr. of records: {}".format(df_copy.shape[0],
    #                                                                                             df.shape[0]))
    ...
    ###############################save results
    print(df.shape)
    df.to_csv("data/results_new_{}.csv".format(name), index=False)
...
def read_acl(min_date=1920.11):
    ...
    with open('data/6.bib', encoding='utf-8') as bibtex_file:
...
# max_date=2023.05
# min_date=2021.11
# last_updated='2021-11-02'#for arxiv scraping
...
if update_data:
    ...
    #read_dblp(min_df_year=2005)  # read the dblp export dump##is uncommented because reading and parsing the whole database takes a while
    read_acl()
