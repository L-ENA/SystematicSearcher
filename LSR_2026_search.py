...
def search_df_3_clusters(df, date_max=2020.03, date_min=2005.00, field="TiAbs", name=""):
    #
    # Method that implements the systematic search. Parameters define restrictions for the publication date. These have to be givin in Float form as above.
    # "field" parameter defines which variables will be searched. TiAbs is a concattenation of titles and abstracts, and can be searched for example with data from the arxiv, but not dblp as dblp onlt stores titles
    #
    ...
    def regex_filter_c3(val):

        found = False

        for reg in cluster_3:#pickup tods
    ...
    df = df[df[field].apply(regex_filter_c3)]
    df_copy.drop(df.index, inplace=True)
    print(
        "Dropped {} records via the third search cluster.\nCurrent nr. of records: {}.\nWriting results to disk.".format(
            df_copy.shape[0], df.shape[0]))
    ###############################save results
    df.to_csv("data/results_{}.csv".format(name))
...
def search_df_one(df, date_max=2020.03, date_min=2005.00, field="TiAbs", name=""):
    #
    # Method that implements the systematic search. Parameters define restrictions for the publication date. These have to be givin in Float form as above.
    # "field" parameter defines which variables will be searched. TiAbs is a concattenation of titles and abstracts, and can be searched for example with data from the arxiv, but not dblp as dblp onlt stores titles
    #
    ...
    df = df[df[field].apply(regex_filter_c4)]  # apply function defined above
    df_copy.drop(df.index, inplace=True)
    print("Dropped {} records via the first search cluster.\nCurrent nr. of records: {}".format(df_copy.shape[0],
                                                                                                df.shape[0]))
    ###df_copy.to_csv("dblp_excluded_first.csv")
...
max_date=2099.05
#For Lena LSR last_updated='2024-06-30'#for arxiv scraping
# min_date=2024.06
...
###############ANGELIKA
# from regex_helper_Angelika import cluster_1, cluster_2, cluster_NOT_titles, cluster_NOT_abstracts
# my_df = load_pickled_db('data/acl.pickle')
# my_df.to_csv("data/all_acl.csv", index=False)
# search_df(my_df, date_max=max_date, date_min=min_date, field='TiAbs', name="acl_ANGELIKA_Jan2026")

###############MEL
# from regex_helper_Mel import cluster_1, cluster_2, cluster_NOT_titles, cluster_NOT_abstracts
# my_df = load_pickled_db('data/acl.pickle')
# my_df.to_csv("data/all_acl.csv", index=False)
# search_df(my_df, date_max=max_date, date_min=min_date, field='TiAbs', name="acl_MEL_Jan2026")


########################LENA
from regex_helper_LS import cluster_1, cluster_2, cluster_3,cluster_4,  cluster_NOT_titles, cluster_NOT_abstracts
# my_df = load_pickled_db('data/acl.pickle')
# my_df.to_csv("data/all_acl.csv", index=False)
# search_df_3_clusters(my_df, date_max=max_date, date_min=min_date, field='TiAbs', name="acl_LENA_a_Jan2026")

my_df = load_pickled_db('data/acl.pickle')
my_df.to_csv("data/all_acl.csv", index=False)
search_df_one(my_df, date_max=max_date, date_min=min_date, field='TiAbs', name="acl_LENA_b_Jan2026")

##############################NADIA
# from regex_helper_Nadia import cluster_1, cluster_2, cluster_NOT_titles, cluster_NOT_abstracts
# my_df = load_pickled_db('data/acl.pickle')
# my_df.to_csv("data/all_acl.csv", index=False)
# search_df(my_df, date_max=max_date, date_min=min_date, field='TiAbs', name="acl_Nadia_Jan2026")
